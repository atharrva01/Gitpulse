...
export function Navbar({ user }: NavbarProps) {
  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-white/[0.05] bg-[#060a06]/90 backdrop-blur-xl">
      <div className="w-full px-4 sm:px-6 lg:px-10 h-14 flex items-center justify-between">

        {/* Logo */}
        <Link
          to={user ? '/dashboard' : '/'}
          className="flex items-center gap-2 font-black text-white tracking-tight shrink-0"
          ...
          <>
            ...
            <div className="hidden md:flex items-center gap-1">
              ...
              <Link to={`/u/${user.login}`} className="flex items-center gap-2 px-2 py-1 rounded-lg hover:bg-white/[0.04] transition-colors group">
                ...
                <span className="text-sm text-slate-400 group-hover:text-white transition-colors hidden lg:block">{user.login}</span>
              </Link>
            ...
            <div className="flex md:hidden items-center gap-2">
              <img src={user.avatar_url} alt={user.login} className="w-7 h-7 rounded-full ring-1 ring-white/10" />
              <button
      ...
      {user && open && (
        <div className="md:hidden border-t border-white/[0.05] bg-[#060a06]/95 px-4 py-3">
          ...
          <div className="border-t border-white/[0.05] pt-3 flex items-center gap-3 px-1">
            <Link to={`/u/${user.login}`} onClick={() => setOpen(false)} className="flex items-center gap-2 flex-1 min-w-0">
              <img src={user.avatar_url} alt={user.login} className="w-8 h-8 rounded-full ring-1 ring-white/10 shrink-0" />
              <span className="text-sm text-white/60 truncate">@{user.login}</span>
            </Link>
            <Link to="/settings" onClick={() => setOpen(false)} className="p-2 rounded-lg text-slate-500 hover:text-white hover:bg-white/[0.04] transition-colors">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
              </svg>
