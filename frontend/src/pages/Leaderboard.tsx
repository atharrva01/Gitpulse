      ...
      <div className="w-full px-4 sm:px-6 lg:px-10 py-8 sm:py-10 max-w-screen-xl mx-auto">
            ...
            {data.map((entry, i) => {
              ...
              return (
                <Link
                  key={entry.login}
                  to={`/u/${entry.login}`}
                  className="flex items-center gap-2 sm:gap-4 rounded-2xl border px-3 py-3 sm:px-4 sm:py-3.5 transition-all hover:border-emerald-500/20 group"
                  style={{
                ...
                >
                  ...
                  <div className="w-6 sm:w-8 shrink-0 text-center">
                    {style ? (
                      <span className="text-base sm:text-lg">{style.label}</span>
                    ) : (
                      <span className="text-xs sm:text-sm font-bold text-white/20">{rank}</span>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1 sm:gap-1.5 mt-0.5">
                      ...
                      <span className="text-[10px] font-semibold truncate" style={{ color: badge.colors.accent + 'cc' }}>
                        {badge.name}
                      </span>
                      <span className="hidden sm:inline text-[10px] text-white/20">·</span>
                      <span className="hidden sm:inline text-[10px] text-white/25 font-mono truncate">@{entry.login}</span>
                    </div>
                  </div>

                  {/* Stats — hidden on mobile, visible from sm */}
                  ...
                  <div className="shrink-0 text-right ml-1 sm:ml-2">
                    <p
        ...
        {!isAuthenticated() && (
          <div className="mt-10 rounded-2xl border border-emerald-500/15 bg-emerald-500/[0.04] p-5 sm:p-6 text-center">
            ...
            <a
              href="/auth/github"
              className="inline-flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-black text-sm font-bold px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl transition-colors"
            >
              Sign in with GitHub
            </a>
          </div>
